		...
		if (TL_MODE == "FE" && \Input::cookie('resolution') != "")
		{
			...
			if($GLOBALS['TL_CONFIG']['maxImageWidth'] > 0 && strpos($strTemplate, 'fe_', 0) !== false)
			{
				if ($strContent != "")
				{
					...
					// pattern for links
					$arrPattern['link'] = '/<a([^>]|(?<=[?])[>])*[>]/i';
					// pattern for style attribute see #2 (thx to tobaco)
					$arrPattern['style'] = '/style="[^"]*["]/i';
					...
					foreach ($arrPattern as $type=>$pattern)
					{
						switch ($type)
						{
							...
							case "link":
								...
								break;
							case "style":
								// Pattern for style see #2 (thx to tobaco)
								$imgPattern = '/background(-image|):.*url\((\'|)(?<file>(files|tl_files)[a-zA-Z0-9%#_:&;\/\.\-\s]{1,}\.(jpe?g|gif|png))(\'|)\).*?("|\')/i';
								$strContent = $this->getFileToReplace($pattern, $imgPattern, $strContent, $type);
								break;
							default:
								break;
